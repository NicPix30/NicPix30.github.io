...
//player unlocks
var doubleGun = new Boolean('false')
//player unlocks
function init() {
	...
	if (!intervalsId)
		{
			intervalsId = setInterval(() => {
				...
				if(Math.random() < 0.5){
				 	x = Math.random() < 0.5 ? 0 - outRadius : 
					canvas.width + outRadius
					y = Math.random() * canvas.height
				} else {
					x = Math.random() * canvas.width
					y = Math.random() < 0.5 ? 0 - outRadius : 
					canvas.height + outRadius
				...
				health.push(new HealthD(x, y, outRadius, inrRadius, inrColor, outColor, velocity));
...
function animate() {
	...
	projectiles.forEach((projectile,projectileIndex) => {		
		health.forEach((healthD, index) => {
			const dist = Math.hypot(healthD.x - projectile.x,
	...
	health.every((healthD, index) => {
		healthD.update()
		const dist = Math.hypot(player.x - healthD.x,
			player.y - healthD.y)
